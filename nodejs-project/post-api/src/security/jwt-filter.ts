...
export class JwtFilter {
    static async verifyToken(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        console.log(token!);
        if(token) {
            await this.authenticateToken(token,req,  res, next);
        } else if(!token) {
            await this.processNonAccessToken(res);
        }
    }

    private static async authenticateToken(token: string, req: Request, res: Response, next: NextFunction): Promise<void> {
        const access_token: string = token.substring(7);
        if(access_token !== 'null'){
            await this.authenticateAccessToken(access_token,req,  res, next);
        } else {
    ...
    private static async authenticateAccessToken(access_token: string, req: Request, res: Response, next: NextFunction): Promise<void> {
